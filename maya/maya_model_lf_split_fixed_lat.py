...
model_save_dir    = os.path.join(os.getcwd(), 'maya_models', 'pn_side_mse_matrix')
os.makedirs(train_save_dir, exist_ok=True)
os.makedirs(model_save_dir, exist_ok=True)
print("Saving to:", model_save_dir)

# For computing class‐specific weights inside masked_MSE_loss:
uncertain_weight_factor = 0.2
neg_cutoff = 0.4
pos_cutoff = 0.6

# ────────────────────────────────────────────────────────────────────────────────
# 2) CUSTOM DATASET: returns (image_tensor, float_label_tensor)
# ────────────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────────────
def get_filtered_df(col, num=None, frontal_status=False):
...
# Build WeightedRandomSampler to slightly balance classes
train_labels_float = train_df[target_col].values  # in {0.0, 0.5, 1.0}
...
class PneumoniaDenseNet(nn.Module):
    def __init__(self, hidden_size: int = 512):
        ...
        # Freeze everything except DenseBlock4 and classifier
...
w0  = total / (counts.get(0.0, 0) + 1)               # weight for Negative
w05 = total / (counts.get(0.5, 0) + 1) * uncertain_weight_factor
w1  = total / (counts.get(1.0, 0) + 1)               # weight for Positive
max_ratio = 5
max_w = max(w0, w05, w1)
...
for epoch in range(num_epochs):
    ...
    # APPEND classification report to log
    with open(log_file_path, 'a') as f:
        f.write(report + "\n")
