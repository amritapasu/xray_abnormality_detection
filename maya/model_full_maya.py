...
target_col = 'Pneumonia' if os.getenv('TARGET_COL')   is None else os.getenv('TARGET_COL')
image_root_dir = "input_images/train_saved_tensors" if os.getenv('IMAGE_ROOT_DIR')   is None else os.getenv('IMAGE_ROOT_DIR')
model_save_dir = 'maya/model_full_pn1' if os.getenv('SAVE_DIR')   is None else os.getenv('SAVE_DIR')

end_df = 10000 if os.getenv('END_DF')   is None else int(os.getenv('END_DF'))
...
hidden_size = 256 if os.getenv('HIDDEN_SIZE')   is None else int(os.getenv('HIDDEN_SIZE'))
lr = 0.0001 if os.getenv('LR')   is None else float(os.getenv('LR'))
weight_decay = 1e-4 if os.getenv('WEIGHT_DECAY')   is None else float(os.getenv('WEIGHT_DECAY'))
freeze_until = 2 if os.getenv('FREEZE_UNTIL')   is None else int(os.getenv('FREEZE_UNTIL'))
...
def freeze_base_layers(model, until_layer=2):
    """
...
filtered_train_df = get_filtered_df(target_col, num=end_df)
